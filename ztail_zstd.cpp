...
void usage(const char* progName) {
    std::cerr << "Uso: " << progName << " [-n N] <arquivo.zst | arquivo.zip>\n"
              << "       " << progName << " [-n N]\n"
...
void readZstdFile(const std::string &filename, CircularBuffer &cb, int n) {
    
    FILE* file = fopen(filename.c_str(), "rb");
    if (!file) {
        throw std::runtime_error("Erro ao abrir arquivo Zstd: " + filename);
    }
    ...
    while ((bytesRead = fread(compressedBuffer.data(), 1, compressedBuffer.size(), file)) > 0) {
        ...
        size_t decompressedSize = ZSTD_decompress(decompressedBuffer.data(), decompressedBuffer.size(),
                                                 compressedBuffer.data(), bytesRead);
        if (ZSTD_isError(decompressedSize)) {
            fclose(file);
            throw std::runtime_error("Erro ao descomprimir bloco Zstd no arquivo: " + filename);
        }

        size_t pos = 0;
        while (pos < decompressedSize) {
    ...
    if (ferror(file)) {
        fclose(file);
        throw std::runtime_error("Erro ao ler arquivo Zstd: " + filename);
    }
        ...
        if (fileIndex != -1) {
            ...
            if (filename.size() >= 4 &&
                ...
                std::cerr << "Extensão não reconhecida em \"" << filename << "\".\n"
                          << "Suporte apenas para .zst e .zip.\n";
                return EXIT_FAILURE;
            }

            if (isZst) {
                readZstdFile(filename, cb, n);
            } else if (isZip) {
